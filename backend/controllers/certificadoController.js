    ...
    try {
        ...
        conexion.query(sqlUsuario, [usuario_id], async (err, userResult) => {
            ...
            conexion.query(sqlCurso, [curso_id], async (err, courseResult) => {
                if (err) return res.status(500).json({ error: err.message });
                if (courseResult.length === 0) return res.status(404).json({ error: 'Curso no encontrado' });
                ...
                const pdfPath = path.join(os.tmpdir(), `test_${usuario_id}_${curso_id}_${Date.now()}.pdf`);
                await generarPDF({ nombre_completo, curso, profesor, fecha, pdfPath });
                ...
                const uploadResult = await cloudinary.uploader.upload(pdfPath, {
                    folder: 'certificados',
                    resource_type: 'auto',
                    public_id: `test_${usuario_id}_${curso_id}`
                });

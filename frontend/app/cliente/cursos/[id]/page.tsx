...
export default function CursoDetallePage() {
  ...
  const fetchData = async () => {
    ...
    const videosConBloqueo = progresoData.progreso.map((video: Video, index: number) => {
      let bloqueado = true;
      if (index === 0) bloqueado = false;
      else if (progresoData.progreso[index - 1]?.visto === true) bloqueado = false;
  ...
  const marcarVisto = async (videoId: number) => {
    ...
    console.log('Marcando video como visto:', videoId);
    ...
    try {
      ...
      if (response.mensaje || response.id) {
        ...
        console.error('Error al marcar:', response.error);
        return false;
      }
    } catch (error) {
      ...
      <div style={{
        ...
        <div>
          {progreso?.progreso.map((video: Video, index: number) => (
            <div
              key={video.id}
              onClick={() => !video.bloqueado && handleVerVideo(video)}
              style={{
                padding: '1rem',
                ...
                backgroundColor: video.visto ? (isDark ? '#0a3b2a' : '#d1fae5') : (video.bloqueado ? (isDark ? '#2a2a2a' : '#f5f5f5') : 'transparent'),
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                ...
                <div>
                  <div style={{
                    fontWeight: video.visto ? 'normal' : '500',
                    color: isDark ? '#fff' : '#333'
                  }}>
                    {video.titulo}
                  </div>
              ...
              <div style={{
                fontSize: '1.2rem',
                color: video.bloqueado ? (isDark ? '#555' : '#ccc') : '#2563eb'
              }}>
                {!video.bloqueado && '▶'}
                {video.bloqueado && '🔒'}
              </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <video
              ...
              onEnded={async () => {
                console.log('Video terminado por reproducción');
                if (videoActual && !videoActual.visto) {
            ...
            <div style={{
              ...
              <button
                onClick={async () => {
                  console.log('Marcando manualmente como completado');
                  if (videoActual && !videoActual.visto) {
                    await marcarVisto(videoActual.id);
                    setMostrarModal(false);
                    setVideoActual(null);
                  ...
                  {[1, 2, 3, 4, 5].map(s => (
                    <button
                      ...
                      style={{
                        ...
                        border: 'none',
                        cursor: 'pointer',
                        color: s <= puntuacion ? '#f59e0b' : (isDark ? '#555' : '#ccc')
                      }}
                    >
                      ★
                    </button>
